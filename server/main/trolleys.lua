...
RegisterServerEvent('av_union:spawnTrolleys', function()
    ...
    if distance <= 20 and heist['vaultOpen'] then
        ...
        banPlayer(src, 'av_union:spawnTrolleys', distance)
    end
end)
...
RegisterServerEvent('av_union:deleteTrolley', function(netId)
    local src = source
    local player = GetPlayerPed(src)
    local distance = #(GetEntityCoords(player) - vector3(-0.2695, -675.1189, 16.1308))
    if distance <= 50 and heist['vaultOpen'] then
        ...
        banPlayer(src, 'av_union:deleteTrolley',distance)
    end
end)